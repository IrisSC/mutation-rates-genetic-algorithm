...
public class GeneticAlgorithm {
	...
	public static void main(String[] args) throws IOException {
        ...
        Knapsack7 sack3 = new Knapsack7();
        Knapsack8 sack2 = new Knapsack8();
		...
		GATestData.add(new Object[] {"Generation", "Max Fitness", "Min Fitness",
				...
				"Avg Mutation Rate", "Mutation Rate Stand Dev"});
		int numValid = 0;
			...
			*/
			...
			for(int i = 0; i < numGenerations; i++) {
				...
				if(i%100 == 0 && i/100 == 1) {
					...
					System.out.println("switch to knapsack 2");
				}else if(i%100 == 0 && i/100 == 2) {
					pop.setKnapsack(sack3);
					System.out.println("switch to knapsack 3");
				}
		...
		for(int i = 0; i < numGenerations; i++) {
			...
			for(int j = 0; j < numRuns; j++) {
				...
				mutateStandDev = mutateStandDev + Math.pow(mutationRate-avgMutationRate, 2);
			}
			...
			mutateStandDev = Math.round(Math.sqrt(mutateStandDev/numRuns)*100.0)/100.0;
			...
			GATestData.add(new Object[] {Integer.toString(i), Double.toString((double)(maxTotal)/(double)numRuns), 
					...
					Double.toString(maxStandDev), Double.toString(avgStandDev), Integer.toString(lastMax), Double.toString(avgMutationRate),
					Double.toString(mutateStandDev)});
